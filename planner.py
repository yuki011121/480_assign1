...
def parse_world(filename):
    ...
    cols = int(lines[0].strip()) # 第一行：列数 [cite: 23]
    rows = int(lines[1].strip()) # 第二行：行数 [cite: 24]
    ...
    for r in range(rows):
        for c in range(cols):
            if grid[r][c] == '@': # 机器人起始位置 [cite: 33]
                robot_start = (r, c)
            elif grid[r][c] == '*': # 脏单元格 [cite: 31]
...
def generate_successors(state, grid):
    ...
    # [cite_start]1. 尝试移动动作 (N, S, E, W) [cite: 9]
    moves = {'N': (-1, 0), 'S': (1, 0), 'W': (0, -1), 'E': (0, 1)}
    for action, (dr, dc) in moves.items():
        new_row, new_col = row + dr, col + dc
        
        # [cite_start]检查移动是否有效（在边界内且不是墙） [cite: 30]
...
def uniform_cost_search(grid, robot_start, dirty_cells):
    ...
    while frontier:
        ...
        for new_state, action in successors:
            if new_state not in visited:
                new_cost = cost + 1 # 所有动作成本为1 [cite: 10]
                new_path = path + [action]
...
def main():
    ...
    # [cite_start]根据要求格式化输出 [cite: 42]
    if path is not None:
        for action in path:
            print(action) # 每行一个动作 [cite: 43]
    else:
        print("No solution found.")
    
    print(f"{generated} nodes generated") # 打印生成的节点数 [cite: 44]
    print(f"{expanded} nodes expanded") # 打印扩展的节点数 [cite: 45]
